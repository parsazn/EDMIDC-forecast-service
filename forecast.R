...
data$createdDate <- as.POSIXct(data$`X_time`, format="%Y-%m-%dT%H:%M:%OSZ", tz="UTC")

# Ensure that the data is sorted by date and time
data <- data %>% arrange(createdDate)

# Create a time series object for the ARIMA model with 1-minute intervals
ts_data <- ts(data$`X_value`, frequency = 1440)  # 1440 intervals per day (1-minute intervals)
...
predict_price <- function(datetime) {
  ...
  # Calculate the number of 1-minute intervals from the last date-time in the dataset to the target date-time
  last_datetime <- max(data$createdDate)
  intervals_to_predict <- as.numeric(difftime(datetime, last_datetime, units = "mins"))  
